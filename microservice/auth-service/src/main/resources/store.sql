-- REGISTER USER
CREATE
OR REPLACE PROCEDURE AUTH_SERVICE.REGISTER_USER (
	OUT O_RES_CODE INT,
	OUT O_RES_ID UUID,
	IN I_EMAIL VARCHAR,
...
-- GET USER BY ID
CREATE
OR REPLACE FUNCTION AUTH_SERVICE.GET_USER_BY_ID (IN I_ID UUID) RETURNS TABLE (
	ID UUID,
	EMAIL VARCHAR(255),
...
SELECT
	*
FROM
	AUTH_SERVICE.GET_USER_BY_ID ('9078599b-e96d-4ad2-9e3e-c1c747ad4f52');

-- GET USER LOGIN
CREATE
OR REPLACE FUNCTION AUTH_SERVICE.GET_USER_LOGIN (IN I_EMAIL VARCHAR, IN I_PASSWORD VARCHAR) RETURNS TABLE (
	ID UUID,
	EMAIL VARCHAR(255),
	PASSWORD VARCHAR(255),
	ROLE VARCHAR(50),
...
-- UPDATE USER
CREATE
OR REPLACE PROCEDURE AUTH_SERVICE.UPDATE_USER (
	OUT O_RES_CODE INT,
	OUT O_ID UUID,
	IN I_ID UUID,
